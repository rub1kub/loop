...
3. PLUSH BRICK ownership проверяется в mainnet, но fee discount и buyback ещё не исполняются
   контрактом или отдельным проверяемым процессом. Onboarding и лендинг с 2026-07-26 называют это
   планом, а не действующим правилом; профиль возвращает `fee_discount_active=false`.
