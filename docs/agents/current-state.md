...
3. PLUSH BRICK fee-free — утверждённая mainnet-модель: тексты обещают её в настоящем времени по
   решению владельца, исполняет DuelEscrow v1.4 (holder-permit). На v1.3 профиль возвращает
   `fee_discount_active=false`. Buyback — ручное намерение без публичного журнала.
