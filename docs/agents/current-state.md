...
| Поле                    | Значение                                   |
| ----------------------- | ------------------------------------------ |
| Проверено               | 2026-07-26 22:10 UTC                       |
| Активный runtime commit | `6f191017584e1b68ff0495a2c89e0b542a1fc625` |
| Активная ветка          | `main`                                     |
...
- release и web release указывают на один commit `6f191017…`;
...
| Поле             | Значение                                                           |
| ---------------- | ------------------------------------------------------------------ |
| Address          | `kQDy0JOVjIjeH-ZmyCv1LLjmJHBJVOicPo0T3atxn5BLNL_q`                 |
| Code hash        | `0FF2E5136C4186BEA8269D426E31FAA9E7D4B8BEBF5BAEFB13B3118F677560ED` |
...
Предыдущий `BankQueue 1.3.0` (`kQCqjhis…`) выведен из обращения: `paused=true`, `locked=0`.
Очередь не пустеет сама — остаток каждого взноса становится наполнением следующей позиции, — но её
удалось привести к нулю намеренно: потребность головы была подготовлена так, чтобы один следующий
взнос покрыл её ровно, не оставив остатка. Обе ожидавшие позиции получили полную целевую выплату,
поэтому в выведенном контракте не осталось ничьих средств. Процедура закреплена скриптом
[`scripts/settle-bank-queue.tolk`](../../scripts/settle-bank-queue.tolk).
...
| Проверка                        | Результат                                  |
| ------------------------------- | ------------------------------------------ |
| Контрактные тесты               | `95/95` в 4 файлах, включая состязательный |
...
контактом и суммой, и **лимит в 1 GRAM вместо 10**. Гейт изначально требовал ещё и не менее 30 дней
в тестовой сети; владелец отменил это требование 2026-08-01 ради скорости запуска, и
`docs/no-audit-disclosure.md` говорит об этом прямо.
