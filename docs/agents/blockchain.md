...
## Проверка сети 2026-08-13

Mainnet-манифесты содержат финализированные deployment и smoke proofs. Рабочая PostgreSQL-проекция
на момент снимка runtime `5242d2e…` видела `113` завершённых DUEL и `180` выплат BANK. Эти
оперативные числа живут в [current-state.md](current-state.md), а не являются контрактными
константами; перед финансовым действием повторно читай getters и сеть.
