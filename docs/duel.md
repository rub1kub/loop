...
Winner payout is `pool - floor(pool × fee / 10,000)`. DuelEscrow V1 enforces one global 2.5% fee.
...
Direct invitations use a random Telegram-safe code. Acceptance reserves the challenge for one Telegram user; the API reloads the offer and checks creator, wallet, state, expiry, pool and complementary chance before returning a contract call.

## Commit–reveal

Each player generates a 256-bit secret locally and funds an offer with its commitment. The secret is kept in secure Telegram/device storage, never in the backend database. Once matched, players reveal within the deadline. The contract combines valid reveals to select a weighted winner.
